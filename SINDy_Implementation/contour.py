...
def contour(Uf,cmp,vml,x,y,fs1=30,cylinder_num=1,markers=None):
  ...
  if markers is not None:
    # Ensure always working with a list of (x, y) tuples
    if isinstance(markers[0], (float, int)):  # single marker
        markers = [markers]
    for marker in markers:
        x_m, y_m = float(marker[0]), float(marker[1])  # force scalars
        plt.plot(x_m, y_m, marker='x', color='red', markersize=8)
  plt.show()
  return
